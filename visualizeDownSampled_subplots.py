...
def plotScatterProjection(data,fullData,fieldNames,lims):
    nDim = data.shape[1]
    if nDim>2:
        fig, axs = plt.subplots(nDim-1, nDim-1,figsize=(12,12), sharex='col',sharey='row')
        for idim in range(nDim-1):
            for jdim in range(idim+1,nDim):
                # plot contours of support of all data
                a = axs[jdim-1,idim].scatter(fullData[:,idim],fullData[:,jdim],color='gray',s=0.2)
                a = axs[jdim-1,idim].scatter(data[:,idim],data[:,jdim],color='blue',s=0.2)
...
lims = [( xmin - 0.05*(xmax - xmin), xmax + 0.05*(xmax - xmin)) for xmin,xmax in zip(mins,maxs)]
...
for nSample in nSamples:
    ...
    plt.savefig(figureFolder+'/'+inpt['prefixDownsampledData']+'_'+str(nSample)+'.png')
    plt.close()
    print('DONE!')
    sys.stdout.flush()
