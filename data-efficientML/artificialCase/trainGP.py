    ...
    for irep in range(5):
        for nPoint in nPoints:
            ...
            for mode in downsSamplingModes:
                ...
                f = open('Max_'+mode+'_n'+str(nPoint)+'_eps'+str(epsilon), "a+")  # append mode 
                f.write(str(np.amax(Error)) + "\n"   )
                f.close()
                f = open('Mean_'+mode+'_n'+str(nPoint)+'_eps'+str(epsilon), "a+")  # append mode 
                f.write(str(np.mean(Error)) + "\n"   )
                f.close()
                f = open('Std_'+mode+'_n'+str(nPoint)+'_eps'+str(epsilon), "a+")  # append mode 
