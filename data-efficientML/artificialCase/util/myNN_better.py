...
class myNN(Model):
    ...
    def train(self, batch_size, epochs, xTrain, yTrain, learningRate, lrScheduler):
        ...
        # Train
        printProgressBar(0, epochs, prefix = 'Loss=%s  Epoch= %d / %d ' % ('?', 0, epochs),suffix = 'Complete', length = 50)
        for epoch in range(epochs):
            ...
            for step, (x_batch_train, y_batch_train),  in enumerate(train_dataset):
                ...
                mse = self.train_step(x_batch_train,y_batch_train,dsOptimizer)
            ...
            printProgressBar(epoch+1, epochs, prefix = 'Loss=%.2f  Epoch= %d / %d ' % (train_mse_loss/nSample, epoch+1, epochs),suffix = 'Complete', length = 50)

            bestLoss = self.logTraining(epoch,mse=train_mse_loss/nSample,bestLoss=bestLoss)
