...
def appendResultToString(string, n, eps, mode, metric):
    if mode == "GP":
        mean1, std1, _, _ = file2stats(
            metric + "_phase_n" + str(n) + "_eps" + str(eps)
        )
        mean2, std2, _, _ = file2stats(
            metric + "_random_n" + str(n) + "_eps" + str(eps)
        )
        mean3, std3, _, _ = file2stats(
            metric + "_kmeans_n" + str(n) + "_eps" + str(eps)
        )
    elif mode == "NN":
        mean1, std1, _, _ = file2stats(
            metric + "NN_phase_n" + str(n) + "_eps" + str(eps)
        )
        mean2, std2, _, _ = file2stats(
            metric + "NN_random_n" + str(n) + "_eps" + str(eps)
        )
        mean3, std3, _, _ = file2stats(
            metric + "NN_kmeans_n" + str(n) + "_eps" + str(eps)
        )
...
print(r"\hline")
print(
    r" Metric & n & $\varepsilon$ & Algo.~\ref{algo:iterative} (2 iter.) & Random & Stratified & Optimum \\ \hline "
)
