...
def getPrediction(model,data):
    ...
    printProgressBar(0, nBatch, prefix = 'Eval ' + str(0) + ' / ' +str(nBatch),suffix = 'Complete', length = 50)
    for ibatch in range(nBatch):
        start_ = startData_b[ibatch]
        end_ = startData_b[ibatch] + nData_b[ibatch]
        result[start_:end_] = np.squeeze(model(data[start_:end_]))
        printProgressBar(ibatch+1, nBatch, prefix = 'Eval ' + str(ibatch+1) + ' / ' +str(nBatch),suffix = 'Complete', length = 50)
...
for epsilon in epsilons:
    ...
    combustion_data_artificial += epsilon*np.random.normal(size=combustion_data_artificial.shape[0])
    for irep in range(5):
        for nPoint in nPoints:
            ...
            for mode in downsSamplingModes:
                ...
                OutputNN = combustion_data_artificial[indices].copy()
                ...
                OutputNN_all = combustion_data_artificial
                ...
                f = open('MaxNN_'+mode+'_n'+str(nPoint)+'_eps'+str(epsilon), "a+")  # append mode 
                f.write(str(np.amax(Error)) + "\n"   )
                f.close()
                f = open('MeanNN_'+mode+'_n'+str(nPoint)+'_eps'+str(epsilon), "a+")  # append mode 
                f.write(str(np.mean(Error)) + "\n"   )
                f.close()
                f = open('StdNN_'+mode+'_n'+str(nPoint)+'_eps'+str(epsilon), "a+")  # append mode 
