...
from uips.utils.typechecks import is_int
...
class SqueezeTransform(Transform):
    ...
    def __init__(self, factor=2):
        ...
        if not is_int(factor) or factor <= 1:
...
class ReshapeTransform(Transform):
    ...
    def forward(self, inputs, context=None):
        ...
        return inputs.reshape(-1, *self.output_shape), torch.zeros(
            inputs.shape[0]
        )
    ...
    def inverse(self, inputs, context=None):
        ...
        return inputs.reshape(-1, *self.input_shape), torch.zeros(
            inputs.shape[0]
        )
