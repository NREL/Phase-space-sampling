...
def plot_probabilityMapDouble2D(model,minX,maxX,minY,maxY,nx=100,ny=100,minval=None,maxval=None):
    ...
    fig=plt.figure()
    plt.imshow(np.transpose(prob), cmap=cm.jet, interpolation='bicubic', vmin=minval, vmax=maxval, extent=[np.amin(x),np.amax(x),np.amax(y),np.amin(y)],aspect='auto')
...
def plot_fromLatentToData(model,nSamples, xfeat=None, yfeat=None):
    ...
    axprettyLabels(axes[0],'feature '+str(xfeat),'feature '+str(yfeat),14,title='Prior')
    axes[1].plot(x[:, xfeat], x[:, yfeat], 'o', markersize=3,color="k")
    axprettyLabels(axes[1],'feature '+str(xfeat),'feature '+str(yfeat),14,title='Generated')
